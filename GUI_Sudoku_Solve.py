...
import numpy as np
from Sudoku_Solver import solve_grid, check_valid, search_blank
pygame.font.init()


N=0
select=input("Enter a difficulty level (HARD , MEDIUM or EASY :")
if(select=='HARD' or select=='Hard' or select=='H'):
    N=180
if select=='MEDIUM' or select=='Medium' or select=='M' :
    N=120
if select=='EASY' or select=='Easy' or select=='E':
    N=60E


grid=np.array(list(str(generators.random_sudoku(avg_rank=int(N)))))
grid=list(map(int,grid))
grid=np.array(grid)
grid=grid.reshape(9,9)
# for i in range(0,len(sudoku_grid)):
#     for j in range(0,len(sudoku_grid[i])):
